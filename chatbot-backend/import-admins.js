// Script to IMPORT all admins from the latest JSON export into a target MongoDB.
...
//
// The script will:
//   - Read the most recent JSON file from ./admin-exports
//   - Upsert documents into the "admins" collection based on email
...
async function getLatestExportFile() {
  const exportDir = path.join(__dirname, 'admin-exports');
...
async function main() {
  try {
    ...
    if (!mongoUri) {
      throw new Error(
        'TARGET_MONGODB_URI is not set. Set it in environment or hard-code it in import-admins.js.'
      );
    }
    ...
    console.log(`Reading admins from: ${filePath}`);

    const raw = fs.readFileSync(filePath, 'utf8');
    const docs = JSON.parse(raw);

    console.log(`Importing ${docs.length} admins (upsert by email)...`);

    for (const doc of docs) {
      const { _id, ...rest } = doc;
      if (!rest.email) {
        console.warn('Skipping admin without email:', doc);
        continue;
      }
